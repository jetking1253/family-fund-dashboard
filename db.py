...
def db_insert_assets_and_nav(hk, us, div, hr, new_total_assets, current_total_shares, new_nav):
    try:
        ...
        # 2. 插入净值快照，返回新插入的 id 作为 snapshot_id
        nav_resp = supabase.table("nav_history").insert({
            "total_assets": new_total_assets,
            "total_shares": current_total_shares,
            "nav": new_nav
        }).execute()
        snapshot_id = nav_resp.data[0]['id'] if nav_resp.data else None
        return True, "资产与净值更新成功！", snapshot_id
    except Exception as e:
        return False, str(e), None

def db_insert_asset_details(snapshot_id, details_list):
    """将 AI 解析或手工填写的持仓明细批量入库。
    details_list: [{"category": "港股", "stock_name": "腾讯", "shares_held": 100, ...}, ...]
    """
    try:
        supabase = get_supabase()
        rows = []
        for item in details_list:
            rows.append({
                ...
                "shares_held": item.get("shares_held") or None,
                "cost_price": item.get("cost_price") or None,
                "current_price": item.get("current_price") or None,
                "total_value": item.get("total_value") or None,
                "pnl": item.get("pnl") or None,
                "pnl_pct": item.get("pnl_pct") or None,
                "notes": item.get("notes", ""),
            })
        if rows:
            supabase.table("asset_details").insert(rows).execute()
        return True, f"成功录入 {len(rows)} 条持仓明细"
    except Exception as e:
        return False, f"明细入库失败: {e}"

def db_get_latest_asset_details():
    """获取最近一次资产快照的持仓明细（按 snapshot_id 倒序取最新的一批）"""
    try:
        supabase = get_supabase()
        # 先获取最新 snapshot_id
        top_resp = supabase.table("asset_details").select("snapshot_id").order("snapshot_id", desc=True).limit(1).execute()
        if not top_resp.data:
            return []
        latest_snapshot_id = top_resp.data[0]['snapshot_id']
        details_resp = supabase.table("asset_details").select("*").eq("snapshot_id", latest_snapshot_id).order("category").execute()
        return details_resp.data
    except Exception as e:
        print(f"查询持仓明细失败: {e}")
        return []

def db_create_asset_details_table():
    """在 Supabase 中通过 REST 方式执行 DDL 创建 asset_details 表（首次部署使用）"""
    try:
        supabase = get_supabase()
        sql = """
        ...
        return True, "asset_details 表创建成功"
    except Exception as e:
        return False, f"建表失败（可能已存在或权限不足，需手动在 Supabase 执行 SQL）: {e}"
